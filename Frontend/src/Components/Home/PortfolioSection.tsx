...
const PortfolioSection = () => {
  ...
  const portfolios = [<FreshFinder />, <SalaryMan />, <Smithsonian />];
  ...
  const handleForward = () => {
    setIndex((next) => (next === portfolios.length - 1 ? 0 : next + 1));
  };

  return (
    <div
      className={`h-[450px] transition-opacity duration-500 ${isVisible ? "opacity-100" : "opacity-0"} text-center`}
    >
      <h2 className="font-semibold py-3 text-slate-600">
        {" "}
        Portfolio Highlights
      </h2>
      <div className="hidden md:flex flex-row justify-center gap-4 p-4">
        {portfolios.map((portfolio, index) => (
          <div key={index}> {portfolio} </div>
        ))}
      </div>
